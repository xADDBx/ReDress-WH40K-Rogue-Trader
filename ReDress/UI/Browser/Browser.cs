...
public class Browser<T> : VerticalList<T> where T : notnull {
    ...
    protected void SearchBarGUI() {
        ...
        void DebouncedSearch() {
            Thread.Sleep((int)(Main.m_Settings.SearchDelay * 1000));
        ...
        Action<(string oldContent, string newContent)>? contentChangedAction = Main.m_Settings.ToggleSearchAsYouType ? (((string oldContent, string newContent) pair) => {
            if ((Time.time - LastSearchedAt) > Main.m_Settings.SearchDelay) {
        ...
        using (HorizontalScope()) {
            ActionTextField(ref CurrentSearchString, m_SearchBarControlName, contentChangedAction, (string query) => {
                StartNewSearch(query);
            });
            Space(5);
            Button(SharedStrings.SearchText, () => StartNewSearch(CurrentSearchString));
        }
    }
