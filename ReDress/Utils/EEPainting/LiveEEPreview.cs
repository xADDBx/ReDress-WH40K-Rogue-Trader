...
public sealed class LiveEEPreview : IDisposable {
        ...
        switch (ev.GetTypeForControl(id)) {
            case EventType.MouseDown:
                if ((ev.button == 0 || ev.button == 2) && rect.Contains(ev.mousePosition)) {
                    ...
                    if (doubleClick) {
                        e.Yaw = 0f; e.Pitch = 0f; e.Zoom = 1f; e.FocusOffset = Vector3.zero;
                    } else {
    ...
    internal void Tick() {
        if (m_Cam == null || m_LiveRT == null) { return; }

        if (m_Dragging && GUIUtility.hotControl == 0) { m_Dragging = false; }

        bool anyVisible = false;
        foreach (var e in m_Entries.Values) {
            if (IsVisible(e)) { anyVisible = true; break; }
        }
    ...
    private string? PickTargetKey() {
        ...
        foreach (var kv in m_Entries) {
            var e = kv.Value;
            if (!e.WantsRender || !IsVisible(e)) { continue; }
    ...
    private bool BuildDollFor(PreviewSpec spec, BaseUnitEntity unit, Character avatar) {
        ...
        try {
            ...
            try { Helpers.UIdCache.Add(m_Doll, unit.UniqueId); } catch (ArgumentException) { }
    ...
    private Bounds ComputeDollBounds() {
        m_HasGeometry = false;
        if (m_DollGo == null) { return new Bounds(m_Park, Vector3.one); }
        ...
        foreach (var r in renderers) {
            ...
            Bounds rb = (r is SkinnedMeshRenderer smr && smr.sharedMesh != null)
                ...
                : r.bounds;
            if (!anyLoose) { loose = rb; anyLoose = true; } else { loose.Encapsulate(rb); }
            if ((rb.center - root).sqrMagnitude > maxCenterDistSqr || rb.extents.magnitude > maxExtent) { continue; }
    ...
    private void Snapshot(Entry e) {
        ...
        if (small != m_LiveRT) { RenderTexture.ReleaseTemporary(small); }
    ...
    private void EvictSnapshotsIfNeeded() {
        ...
        foreach (var kv in m_Entries.OrderBy(kv => kv.Value.LastDrawnFrame)) {
            ...
            var e = kv.Value;
            if (e.Snapshot == null || kv.Key == m_LiveKey || kv.Key == m_ActiveKey) { continue; }
    ...
    private void OnBeginCameraRendering(ScriptableRenderContext context, Camera cam) {
        if (m_Cam == null || cam != m_Cam) { return; }
        ...
        try {
            ...
            if (m_Key != null) { m_Key.enabled = true; }
            if (m_Fill != null) { m_Fill.enabled = true; }
    ...
    private void RestoreRenderState() {
        if (!m_RenderStateSwapped) { return; }
        m_RenderStateSwapped = false;
        foreach (var l in m_TempDisabledLights) {
            if (l != null) { l.enabled = true; }
        }
        m_TempDisabledLights.Clear();
        if (m_Key != null) { m_Key.enabled = false; }
        if (m_Fill != null) { m_Fill.enabled = false; }
    ...
    private void ClearEntries() {
        foreach (var e in m_Entries.Values) {
            if (e.Snapshot != null) { UnityEngine.Object.Destroy(e.Snapshot); }
        }
        m_Entries.Clear();
        m_ActiveKey = null;
        m_LastClickKey = null;
